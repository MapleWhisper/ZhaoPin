	...
	public List<Apply> findByState(String state, int month) {
		...
		if(state.equals("已完成") || state.equals("已拒绝")){
			if(month ==1 ){		//如果为一个月，默认返回20条记录
				list= (List<Apply>) session.createQuery("from Apply a where a.state = ?  order by id desc")
						.setParameter(0, state)
						.setMaxResults(20).list();
			}else if(month <=12){		//如果为一年以内的申请
				Calendar calendar = Calendar.getInstance();
				calendar.add(Calendar.MONTH, -month);
				list= (List<Apply>) session.createQuery("from Apply a where a.state = ? and a.applyDate >?  order by id desc")
						.setParameter(0, state)
			...
			list= (List<Apply>) session.createQuery("from Apply a where a.state = ? order by id desc").setParameter(0, state).list();
		}
